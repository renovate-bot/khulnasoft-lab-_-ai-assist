...
from codesuggestions.api.rollout.model import ModelRollout, ModelRolloutPlan
...
@pytest.mark.parametrize(
    ("rollout_percentage", "project_id", "model", "f_third_party_ai_default", "is_third_party_ai_default"),
    [
        ...
        (80, 2670515, ModelRollout.GITLAB_CODEGEN, True, True),  # Does not fall in the rollout window
        ...
        (80, 45504304, ModelRollout.GITLAB_CODEGEN, False, False), # third-party models disabled
        (80, 45504302, ModelRollout.GOOGLE_CODE_GECKO, False, False), # Should be codegen, but user has limited 3rd party access
    ],
)
def test_model_router(rollout_percentage, project_id, model, f_third_party_ai_default, is_third_party_ai_default):
    user = GitLabUser(authenticated=True, claims=UserClaims(is_third_party_ai_default=is_third_party_ai_default))

    router = ModelRolloutPlan(
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (123, {111: Project(111, "")}, False), # Project ID is not in `f_limited_access_third_party_ai`
...
@pytest.mark.parametrize(
    ...
    [
        (
            GitLabUser(authenticated=True, is_debug=True, claims=UserClaims(is_third_party_ai_default=False)), 
            True, 
            True
        ),
        (
            GitLabUser(authenticated=True, is_debug=False, claims=UserClaims(is_third_party_ai_default=True)), 
            True, 
            True
        ),
        (
            GitLabUser(authenticated=True, is_debug=True, claims=UserClaims(is_third_party_ai_default=True)), 
            True, 
            True
        ),
        (
